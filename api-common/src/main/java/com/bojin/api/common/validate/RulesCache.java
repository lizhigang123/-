...
public class RulesCache {
	...
	private static Map<String, ValidRules> annotationValidRules = new HashMap<String, ValidRules>();
	...
	public static ValidRules getAnnotationValidRulesByMethod(Method method) {
		ValidRules validRules = null;
		if (method != null) {
			validRules = annotationValidRules.get(method.getDeclaringClass().getName() + "." + method.getName());
			if (validRules == null) {
				validRules = method.getAnnotation(ValidRules.class);
				annotationValidRules.put(method.getDeclaringClass().getName() + "." + method.getName(), validRules);
			}
		}
		return validRules;
	}
